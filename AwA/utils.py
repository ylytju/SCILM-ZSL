...
def data_iterator(sess, train_data, selected_num, img, att, weights):
    ...
    for i in range(len(unique_train_label)):
        ...
        mean_fea = 0.4*mean_fea_a + 0.6*mean_fea_b
...
def compute_accuracy(gen_img, test_data):
    ...
    outpre_cla = np.zeros(unique_labels.shape[0])
    acc = 0
    for i in range(int(unique_labels.shape[0])):
        idx = np.nonzero(test_label == unique_labels[i])[0]
        temp_acc = accuracy_score(test_label[idx], outpre[idx])
        outpre_cla[i] = temp_acc
        acc+=temp_acc
    acc = acc / unique_labels.shape[0]
    return acc,outpre_cla
...
def kNNClassify(newInput, dataSet, labels, k):
    ...
    distance_euc = 0.1*squaredDist ** 0.5

    distance_cos = [0] * dataSet.shape[0]
    for i in range(dataSet.shape[0]):
        distance_cos[i] = 1*cosine_distance(newInput, dataSet[i])
